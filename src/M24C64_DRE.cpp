...
M24C64_DRE::M24C64_DRE(const std::shared_ptr<I2C_bus_base>& bus, const long id) : I2C_dev_base(bus, id)
{
...
bool M24C64_DRE::read(const size_t addr, void* buf, const size_t size)
{
	std::shared_ptr<I2C_bus_open_close> bus_closer = std::make_shared<I2C_bus_open_close>(*m_bus);
	...
	addr_buf[0] = (addr & 0xFF00U) >> 8;
	addr_buf[1] = (addr & 0x00FFU) >> 0;
...
bool M24C64_DRE::write(const size_t addr, const void* buf, const size_t size)
{
	std::shared_ptr<I2C_bus_open_close> bus_closer = std::make_shared<I2C_bus_open_close>(*m_bus);

	SPDLOG_DEBUG("M24C64_DRE::write 0x{:02X} {:d}@0x{:04X}", m_dev_addr, size, addr);
...
bool M24C64_DRE::fill(const uint8_t val)
{
	std::shared_ptr<I2C_bus_open_close> bus_closer = std::make_shared<I2C_bus_open_close>(*m_bus);
	...
	for(size_t i = 0; i < size; i += get_pagesize())
	{
		...
		SPDLOG_DEBUG("write 0x{:02X} {:d}@0x{:04X}", m_dev_addr, get_pagesize(), curr_addr);

		write_buf[0] = (curr_addr >> 8) & 0xFFU;
		write_buf[1] = (curr_addr >> 0) & 0xFFU;
...
bool M24C64_DRE::write_page(const size_t addr, const void* buf, const size_t size)
{
	...
	{
		SPDLOG_ERROR("M24C64_DRE::write_page not alligned: {:d}@0x{:04X}", size, addr);
		return false;
	}

	if( size > get_pagesize() )
	{
		SPDLOG_ERROR("M24C64_DRE::write_page size larger than get_pagesize()");
		return false;
	}

	// gather IO not supported by driver and/or FT260S
	Writebuffer write_buf;
	write_buf[0] = (addr & 0xFF00U) >> 8;
	write_buf[1] = (addr & 0x00FFU) >> 0;
...
bool M24C64_DRE::wait_write_complete()
{
#if 1
